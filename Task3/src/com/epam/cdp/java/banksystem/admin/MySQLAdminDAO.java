...
public class MySQLAdminDAO implements AdminDAO {
	...
	public int createAccount(long userId, long currencyId, double value, Connection conn) throws SQLException {
		PreparedStatement st = conn.prepareStatement("insert into account (user_id, currency_id, value) values(?,?,?)");
		st.setLong(1, userId);
	...
	public Account fetchAccount(long accountId, Connection conn) throws SQLException {
		PreparedStatement st = conn.prepareStatement("select * from account a join user u on a.user_id = u.user_id join currency c on a.currency_id = c.currency_id join role r on u.role_id = r.role_id where a.account_id = ?");
	...
	public List<Account> fetchAccountList(Connection conn) throws SQLException {
		PreparedStatement st = conn.prepareStatement("select * from account a join user u on a.user_id = u.user_id join currency c on a.currency_id = c.currency_id join role r on u.role_id = r.role_id");
		ResultSet rs = st.executeQuery();
	...
	public List<Account> fetchUserAccountList(long userId, Connection conn) throws SQLException {
		PreparedStatement st = conn.prepareStatement("select * from account a join user u on a.user_id = u.user_id join currency c on a.currency_id = c.currency_id join role r on u.role_id = r.role_id where u.user_id = ?");
		st.setLong(1, userId);
		ResultSet rs = st.executeQuery();
		List<Account> accountList = new ArrayList<Account>();
		while (rs.next()) {
	...
	public List<Currency> fetchCurrencyTypeList(Connection conn) throws SQLException {
		PreparedStatement st = conn.prepareStatement("select * from currency");
		ResultSet rs = st.executeQuery();
	...
	public List<User> fetchUserList(Connection conn) throws SQLException {
		PreparedStatement st = conn.prepareStatement("select * from user u join role r on u.role_id = r.role_id where r.role != 'admin'");
		ResultSet rs = st.executeQuery();
