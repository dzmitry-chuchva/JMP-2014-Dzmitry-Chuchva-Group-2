...
public class MySQLCustomerDAO implements CustomerDAO {
	...
	public Conversion fetchConversionRate(long fromId, long toId, Connection conn) throws SQLException {
		PreparedStatement st = conn.prepareStatement("select cur1.currency_id 'from_id', cur1.type 'from_type', cur2.currency_id 'to_id', cur2.type 'to_type', con.rate from conversion con join currency cur1 on con.from_id = cur1.currency_id join currency cur2 on con.to_id = cur2.currency_id where cur1.currency_id = ? and cur2.currency_id = ?");
		st.setLong(1, fromId);
		st.setLong(2, toId);
	...
	public int updateAccountValue(long accountId, double value, Connection conn) throws SQLException {
		PreparedStatement st = conn.prepareStatement("update account set value = value + ? where account_id = ?");
