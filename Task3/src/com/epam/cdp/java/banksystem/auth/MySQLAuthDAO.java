...
public class MySQLAuthDAO implements AuthDAO {
	...
	public User fetchUserByLoginAndPass(String login, String pass, Connection conn) throws SQLException {
		PreparedStatement st = conn.prepareStatement("select * from user u left join role r on u.role_id = r.role_id where u.login = ? and u.password = ?");
	...
	public int createUser(String firstName, String lastName, String login, String pass, Connection conn) throws SQLException {
		PreparedStatement st = conn.prepareStatement("insert into user (role_id, first_name, last_name, login, password) values (?,?,?,?,?)");
