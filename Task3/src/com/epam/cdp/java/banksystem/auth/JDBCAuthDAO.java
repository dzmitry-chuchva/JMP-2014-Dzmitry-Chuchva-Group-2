...
public class JDBCAuthDAO implements AuthDAO {
	...
	PreparedStatement st = conn
		.prepareStatement("select * from banksystem.user u left join banksystem.role r on u.role_id = r.role_id where u.login = ? and u.password = ?");
	...
	PreparedStatement st = conn
		.prepareStatement("insert into banksystem.user (role_id, first_name, last_name, login, password) values (?,?,?,?,?)");
